...
class Generator {
  ...
  // ************************ Internal helpers ************************
  int _getMaxCharsPerLine(List<int>? font) {
    if (_paperSize == PaperSize.mm58) {
      return (font == null || font == EscFont.defaults) ? 32 : 42;
    } else {
      return (font == null || font == EscFont.defaults) ? 48 : 64;
    }
  }
  ...
  /// (even after resetting)
  List<int> setGlobalFont(List<int>? font, {int? maxCharsPerLine}) {
    List<int> bytes = [];
    _font = font;
    if (font != null) {
  ...
  List<int> setStyles(PosStyles styles, {bool isKanji = false}) {
    ...
    if (styles.align != _styles.align) {
      if (_profile.manufacturer == "Epson") {
        bytes += latin1.encode(styles.align == PosAlign.left ? cAlignLeft : (styles.align == PosAlign.center ? cAlignCenter : cAlignRight));
      } else {
        bytes += styles.align == PosAlign.left ? EscAling.left : (styles.align == PosAlign.center ? EscAling.center : EscAling.rigth);
      }
