...
class CapabilityProfile {
  CapabilityProfile._internal(this.name, this.codePages,this.manufacturer);

  /// Public factory
  static Future<CapabilityProfile> load({String name = 'default'}) async {
    final content = await rootBundle.loadString('resources/capabilities.json');
    ...
    return CapabilityProfile._internal(name, list,profile['manufacturer'].toString());
  }
  ...
  static Future<List<dynamic>> getAvailableProfile({String name = 'default'}) async {
    final content = await rootBundle.loadString('resources/capabilities.json');
    Map capabilities = json.decode(content);
    var profiles = capabilities['profiles'];
